...
from paciente.models import Consultas, Documento

def cadastro_medico(request):

    if is_medico(request.user):
        messages.add_message(request, constants.WARNING, 'Você já esta cadastrado como Medico')
    ...
    if request.method == "GET":
        ...
        messages.add_message(request, constants.SUCCESS, 'Cadastro medico efetuado com sucesso')
...
def consultas_medico(request):
    ...
    consultas_hoje = Consultas.objects.filter(data_aberta__user=request.user).filter(data_aberta__data__gte=hoje).filter(data_aberta__data__lt=hoje + timedelta(days=1))
    consultas_restantes = Consultas.objects.exclude(id__in=consultas_hoje.values('id'))
...
def consulta_area_medico(request, id_consulta):
    ...
    if request.method == "GET":
        consulta = Consultas.objects.get(id=id_consulta)
